...
async def animate_spaceship(canvas, row, column, canvas_height, canvas_width):
    ...
    for frame in cycle(frame_sequence):
        ...
        if space:
            fire_coro = fire(canvas, current_y_limited, current_x_limited + 2, -0.9)
...
async def fill_orbit_with_garbage(canvas, canvas_height, canvas_width):
    garbage_frames= ['duck.txt', 'hubble.txt', 'lamp.txt', 'trash_large.txt', 'trash_small.txt', 'trash_xl.txt']
    garbage_list = []
...
def draw(canvas):
    ...
    spaceship_coro = animate_spaceship(
        canvas, 
        canvas_height/2, 
        canvas_width/2,  # substrating 2 is to match the fire shot with the rocket at the first moment 
        canvas_height, 
        canvas_width, 
    )  
